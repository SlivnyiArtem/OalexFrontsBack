...
def build_clasterization_time_window(start_date, end_date, mailto="{EMAIL}", theme_index="T11636"):
    ...
    log(f"Done paging through results. We made {count_api_queries} API queries, and retrieved {len(all_results)} results.")
    ...
    log(f"Вычисляем PageRank для {n} работ...")
    log(f"Параметры: damping={settings.DAMPING_FACTOR}, max_iterations={settings.MAX_ITERATIONS}")

    log("Создаем матрицу переходов...")
    ...
    log("Запускаем итерационный расчет...")
    pagerank = [1.0 / n] * n
    ...
    for iteration in range(settings.MAX_ITERATIONS):
        ...
        if diff < settings.TOLERANCE:
            log(f"✓ PageRank сошелся после {iteration + 1} итераций (diff: {diff:.8f})")
            break

        pagerank = new_pagerank

        if iteration % 10 == 0:
            log(f"  Итерация {iteration}: diff = {diff:.6f}")

    log("Сохраняем результаты PageRank...")
    for idx, work_id in enumerate(works):
        citation_matrix[work_id]['pagerank'] = pagerank[idx]

    log("✓ PageRank успешно вычислен!")
